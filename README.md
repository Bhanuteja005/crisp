...
- **Welcome Back Modal**: Smart session restoration promptsct — here’s a **professional, start-to-finish project plan** you can follow in VS Code (with GitHub + Copilot) to build **AI-Powered Interview Assistant (Crisp)**. I break it into clear milestones, deliverables, folder structure, key components, data models, timelines (sprints), implementation notes, testing, deployment, and a video + README checklist so you can deliver fast and clean.

# Project summary (one line)

Build a React app with two synced tabs — **Interviewee (chat)** and **Interviewer (dashboard)** — with resume upload + parsing, timed interview flow (6 questions), local persistence (restore on reload), scoring & summaries generated by AI, and a dashboard to review candidates.

---

# High-level milestones (7 total)

1. **Project setup & scaffold** — repo, toolchain, UI library, state management.
2. **Resume upload & parsing** — PDF / DOCX ingestion + field extraction (Name/Email/Phone).
3. **Interviewee chat UI + interview engine** — question generation, timers, auto-submit.
4. **Interviewer dashboard** — list, details, search, sort.
5. **Persistence & session restore** — redux-persist / IndexedDB, "Welcome Back" modal.
6. **AI integration & scoring** — question generation, judge answers, final summary.
...
(You can do each sprint in ~2–5 days depending on time available)

**Sprint 1 — Setup & Resume Upload (2–3 days)**
...
**Sprint 2 — Interview Engine + Chat UI (3–4 days)**
...
**Sprint 3 — Interviewer Dashboard & Persistence (3–4 days)**
...
**Sprint 4 — AI Integration, Polish, Tests, Deploy (3–4 days)**
...
├─ public/
├─ src/
│  ├─ api/                 # api wrappers for AI, resume parsing helper
│  ├─ app/
│  │   └─ store.ts
│  ├─ features/
│  │   ├─ interviewee/
...
* Parsing failure: show "Could not parse — please upload a PDF with selectable text or paste your contact info manually."
* AI API errors: show friendly message and fallback to local static questions + mark evaluation as "Unavailable — API error".
...
* Integration tests:
  ...
  * full interview flow from upload → 6 Qs → final score.
  * reload mid-question → restore and continue.
* Manual QA:
...
2. Add `.env.local` for API keys (OpenAI_KEY) — do **not** commit secrets.
3. Push to GitHub.
...
1. 0:00–0:10 — Title slide + quick one-line purpose.
2. 0:10–0:40 — Show Interviewee tab: upload resume, parsed fields auto-filled.
3. 0:40–1:40 — Run through interview: show timer, answer, auto-submit, progress.
4. 1:40–2:10 — Show pause/refresh → Welcome Back modal → resume.
5. 2:10–2:40 — Switch to Interviewer dashboard: candidate list, sort by score, open candidate details (Q/A + per-question AI feedback + final summary).
6. 2:40–3:00 — Closing: Link to GitHub, live demo, and submission note.
...
* [ ] Demo video recorded & linked (2–5 min).
...
Tell me which of those you want next and I’ll produce detailed code and files ready for Copilot to autocomplete.
